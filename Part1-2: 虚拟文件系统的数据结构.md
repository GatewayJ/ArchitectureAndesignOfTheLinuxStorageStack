...
文件夹充当一个用户文件的目录或者容器。对目录适用的操作与普通文件不同。在目录上要使用不同的命令， 文件总是在目录里面，访问一个文件的时候 你需要指定一个相对的或者绝对的路径，但是对于大多数linux 目录也是一个文件，那么他是如何工作的那？

本地的linux文件系统认为目录也是一个文件，并且向文件一样存储他们。像其他常规iu的文件一样，目录也被分配一个inode。在文件和目录的inode上有一个不同，在目录的情况下，inode中的类型字段是目录。请记住，从我们关于inode讨论中得知，inode包含了关于文件的大量元数据，但它不包含文件名。文件名存在于目录中。可以将目录视为包含表格的特殊文件。该表格包含文件名及其相应的索引节点号。
...
- /cars目录将包含三个文件（McLaren、Porsche和Lamborghini）与它们的 inode 号之间的映射。从这里，我们可以使用McLaren的 inode，它将指向包含文件数据的磁盘数据块。
...
- d_lock：此字段包含一个自旋锁，用于保护对struct dentry对象的访问。dentry和inode对象通常被多个进程共享，这些进程打开相同的文件或目录。d_lock字段保护这些对象，避免并发修改，防止导致文件系统数据不一致或损坏。
...
能够被dentry 对象上执行的文件系统相关的操作被定义在dentry_operations结构体内

```c
struct dentry_operations {
 int (*d_revalidate)(struct dentry *, unsigned int);
40 Explaining the Data Structures in a VFS
 int (*d_weak_revalidate)(struct dentry *, unsigned int);
 int (*d_hash)(const struct dentry *, struct qstr *);
 int (*d_compare)(const struct dentry *, unsigned int, const
char *, const struct qstr *);
...
- 内核首先检查所需的数据是否已经存在于缓存中。如果数据在缓存中找到了，内核可以避免执行任何磁盘操作，直接将请求的数据提供给进程。这种情况被称为缓存命中。

- 如果请求的数据未能在缓存中找到，内核必须去底层磁盘进行操作。这被称为读取操作，从磁盘获取请求的数据，保存到缓存中，然后将其交给调用进程。

- 如果之后有更多请求访问此页面，它们可以从页面缓存中获取。

- 在请求的数据已在缓存中找到但已标记为脏的情况下，内核会先将其写回磁盘，然后再执行之前提到的过程。

类似地，当进程需要将数据写入磁盘时，会发生以下情况：

- 内核更新映射到文件的页面缓存，并将数据标记为脏。尚未写入磁盘的页面被称为脏页面。

- 内核不会立即将所有脏数据写入磁盘。根据内核刷写线程的配置，脏数据会被刷新到磁盘。

在写入请求完成后，内核会向调用进程发送确认消息。然而，它不会告知进程对应的脏页面何时会被实际写入磁盘。值得注意的是，这种异步处理使得写入操作比读取操作要快得多，因为内核避免了访问底层物理磁盘的操作。这也引出了一个问题：如果我的 I/O 请求来自内存，在突发性断电的情况下，我的数据会怎么样？ 内存中的脏页面在一定时间后会被刷新到物理磁盘，这个过程称为sysctl，可以用来控制页面缓存的这一行为。
